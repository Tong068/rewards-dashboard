...
export function lineChart(
  ...
  const dots = points
    .map((p, i) => {
      const [x, y] = coords[i];
      const title = t("chart.dotTitle", {
        label: p.label || p.key,
        value: fmtNumber(p.value),
      });
  ...
  container.innerHTML = `
        <svg class="chart" viewBox="0 0 ${width} ${height}" width="100%" height="${height}" role="img"
             aria-label="${escapeAttr(t("chart.lineAria"))}">
            ${grid}
...
export function barChart(
  ...
  const rects = bars
    .map((b, i) => {
      ...
      const title = t("chart.barTitle", {
        date: localDateLabel(b.key, {
          weekday: "short",
          month: "short",
          day: "numeric",
        }),
  ...
  container.innerHTML = `
        <svg class="chart" viewBox="0 0 ${width} ${height}" width="100%" height="${height}" role="img"
             aria-label="${escapeAttr(t("chart.barAria"))}">
            ${grid}
...
export function buildHeatmapHtml(days) {
  ...
  for (let week = 0; week < totalWeeks; week++) {
    ...
    for (let day = 0; day < 7; day++) {
      ...
      const dayLabel = localDateLabel(key, {
        weekday: "long",
        month: "short",
        day: "numeric",
      });
      const title = entry
        ? t("chart.heatTitle", {
          date: dayLabel,
          gained: fmtNumber(entry.gained),
          total: fmtNumber(entry.lastTotal),
        })
        : dayLabel;
...
export function buildAccumBarHtml(days, globalMaxGained, maxDays = null) {
  ...
  const flushWeekMarker = () => {
    ...
    html += `<div class="accum-week-marker" title="${escapeAttr(t("chart.weekMarker", { label, pts: fmtNumber(weekGained) }))}"></div>`;
    weekGained = 0;
  };
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (i > 0) {
      ...
      if (missed >= 1) {
        ...
        html += `<div class="accum-gap${gapClass}" style="width:${missed * 16}px" title="${escapeAttr(t("chart.gap", { n: missed }))}"></div>`;
      }
    }
    ...
    const dayTitle = t("chart.dayTitle", {
      date: dateLabel,
      today: isToday ? t("chart.todaySuffix") : "",
      gained: fmtNumber(day.gained),
      total: fmtNumber(day.lastTotal),
    });

    html += `<div class="accum-day ${altToggle ? "accum-day--a" : "accum-day--b"}${isToday ? " accum-day--today" : ""}" style="height:${heightPct}%;opacity:${opacity}" title="${escapeAttr(dayTitle)}"></div>`;
  }
